...
int BytesPremableDumper(DumpContext_& dc)
{
  if (dc.numeric_preamble_dumped) { 
    bytes += 2;
  } else {
...
#define BYTESPREAMBLEDUMPERDEFINE(NAME) int BytesPreamble##NAME(DumpContext_& dc) { int bytes = 0;  \
...
#define PREAMBLEDUMPERDEFINE(NAME) inline void PreambleDumper##NAME(DumpContext_& dc) { if (dc.NAME##_dumped) { \
    ...
    int len=sizeof(NAME##Preamble)-1; \
...
inline int BytesToDump4ByteInteger () { return 5; }
...
inline void dump8ByteInteger (int_8 i8, DumpContext_& dc)
{
  ...
  if ((buff[7]==0) && (buff[6]==0) && (buff[5]==0) && (buff[4]==0) && 
    ...
    *mem++ = 'L'; // older protocol ...
    const int len = i8_as_string.length();
    const char* data = i8_as_string.data();
    for (int ii=0; ii<len; ii++) *mem++=*data++;
    *mem++ = 'L';
    *mem++ = '\n';
  } else {
...
inline int BytesToDump8ByteInteger (DumpContext_& dc) 
...
inline int BytesToDump8ByteUnsignedInteger (DumpContext_& dc) 
...
inline int BytesToDump8ByteFloat () { return 9; }
...
inline int BytesToDump16ByteComplex (DumpContext_& dc) 
{
  int bytes = BytesToDump8ByteFloat()*2 + 2 + 1; // floats*2 + TUPLE2 + REDUCE
  if (!dc.complex_preamble_dumped) { 
...
inline int BytesToDumpBool () { return 1+1; }


inline void dumpCString (const char* cstr, int len, DumpContext_& dc)
{
...
inline int BytesToDumpCString (const char* /*cstr*/, int bytes)
{
  if (bytes>255) {
    bytes += 5;
  } else {
...
inline int BytesToDumpString (const string& s)
{
  const int len = s.length();
...
inline int BytesToDumpString (const OCString& s)
{
  const int len = s.length();
  return BytesToDumpCString(s.data(), len);
}
...
template <class OT>
int TopLevelBytesToDump (const OT& v, ArrayDisposition_e dis=AS_LIST,
			 PicklingIssues_e issues = ABOVE_PYTHON_2_2)
{
...
template <class OT>
int P2BytesToDumpSingleVal (const OT& v, ArrayDisposition_e dis=AS_LIST,
			    PicklingIssues_e issues = ABOVE_PYTHON_2_2)
{
  DumpContext_ dc(0, dis, issues);
...
template <class STR>
inline void finishLoadingString_ (STR* sp, int len, LoadContext_& lc)
{
...
{
  int len = loadStringLength_(lc);
  finishLoadingString_(sp, len, lc);
}

template <class STR>
inline void finishLoadingString_ (STR& s, int len, LoadContext_& lc)
{
  s=STR(lc.mem, len);
  lc.mem += len;
